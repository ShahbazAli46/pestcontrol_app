import 'package:accurate/AccountantDashboard/Controllers/ReceivedAmountController.dart';
...
class DailyReceiveAmountScreen extends StatefulWidget {
  const DailyReceiveAmountScreen({super.key});

  @override
  State<DailyReceiveAmountScreen> createState() => _DailyReceiveAmountScreenState();
}

class _DailyReceiveAmountScreenState extends State<DailyReceiveAmountScreen> {

  late ReceivedAmountController controller;
  ...
  void initState() {
    ...
    super.initState();
    controller = Get.put(ReceivedAmountController());
  }
  @override
  void dispose() {
    // TODO: implement dispose
    super.dispose();
    Get.delete<ReceivedAmountController>();
  }
    ...
    return Scaffold(
      ...
      body: SafeArea(child: Column(
        children: [
          NavWithBack(),
          SizedBox(height: 20,),
          AppTextLabels.boldTextShort(label: "Received Amount", color: AppColors.appBlack, fontSize: 20),
          SizedBox(height: 20,),
  ...
  _buildItem(index){
    return UiHelper.commonContainer(Column(
      children: [
        ...
        UiHelper.buildRow("Client", "${controller.data[index].referenceable?.name ?? ""}"),
        UiHelper.buildRow("Received By:", "${controller.data[index].personable?.name ?? ""}"),
        UiHelper.buildRow("Amount", "${UiHelper.formatUAECurrency(double.parse(controller.data[index].crAmt ?? ""))}"),
